...
[Makefile.connectal](../Makefile.connectal) and define some `make`
variables.
...
---------------------------------
    ...
    BSVFILES = ...
    CPPFILES = ...
    include /scratch/connectal/Makefile.connectal
---------------------------------
...
XBSV supports Android on Zynq platforms, so connectalgen generates `jni/Android.mk` for `ndk-build`.
