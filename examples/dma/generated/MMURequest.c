...
{
    volatile unsigned int* temp_working_addr_start = p->item->mapchannelReq(p, CHAN_NUM_MMURequest_region, 14);
    volatile unsigned int* temp_working_addr = temp_working_addr_start;
    ...
    p->item->send(p, temp_working_addr_start, (CHAN_NUM_MMURequest_region << 16) | 14, -1);
    return 0;
};
...
int MMURequest_handleMessage(struct PortalInternal *p, unsigned int channel, int messageFd)
{
    ...
    case CHAN_NUM_MMURequest_region: {
        ...
        p->item->recv(p, temp_working_addr, 13, &tmpfd);
        tmp = p->item->read(p, &temp_working_addr);
