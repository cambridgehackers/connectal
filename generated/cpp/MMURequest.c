...
{
    volatile unsigned int* temp_working_addr = p->item->mapchannelReq(p, CHAN_NUM_MMURequest_sglist);
    ...
    p->item->send(p, (CHAN_NUM_MMURequest_sglist << 16) | 6, -1);
};
...
{
    volatile unsigned int* temp_working_addr = p->item->mapchannelReq(p, CHAN_NUM_MMURequest_region);
