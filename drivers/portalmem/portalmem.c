					  ...
					  unsigned long align)
{
  ...
  long size_remaining = PAGE_ALIGN(len);
  struct page *page;
  int i = 0;
  ...
  while (size_remaining > 0) {
    ...
    for (; ordindex < ARRAY_SIZE(orders); ordindex++) {
      ...
      if (size_remaining < (PAGE_SIZE << orders[ordindex]) || max_order < orders[ordindex])
        continue;
      page = alloc_pages(gfp_flags, orders[ordindex]);
      if (page) {
  ...
  if (info) {
    ...
    if (table) {
      int ret = sg_alloc_table(table, i, GFP_KERNEL);
      if (!ret) {
        sg = table->sgl;
...
{
  return ((struct pa_buffer *)dmabuf->priv)->vaddr + offset * PAGE_SIZE;
}
...
#error("PA_DCACHE_FLUSH_INVAL architecture undefined");
  ...
  case PA_ALLOC: {
    ...
    header.size = round_up(header.size, align);
    buffer = pa_buffer_create(PAGE_ALIGN(header.size), align);
    if (!buffer)
      return -ENODEV;
    if (IS_ERR(buffer))
    ...
    header.numEntries = buffer->sg_table->nents;
